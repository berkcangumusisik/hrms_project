...
public interface EducationInformationDao extends JpaRepository<EducationInformation, Integer>{
	@Query("Select new kodlamaio.hrms.entities.dtos.EducationInformationWithCvWithUniversityWithUniversityDepartmentDto("
			+ "e.educationInformationId,f.firstName, f.lastName, c.cvId, u.universityId, "
			+ "u.universityName, d.universityDepartmentId, d.universityDepartmentName, e.startDate, e.graduationDate) "
			...
			+ " Inner Join e.cv c "
			+ " Inner Join JobSeeker f ON e.cv.jobSeeker.userId=f.userId"
